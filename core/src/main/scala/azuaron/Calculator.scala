...
object Calculator {
  ...
  def parse(str: Seq[Char]): (Option[Expr], Seq[Char]) = additive.parse(str)

  def additive: Rule[Expr] = multiplicative ~ ((keyword("+") | keyword("-")) ~ multiplicative).* >> {
  ...
  def multiplicative: Rule[Expr] = unary ~ ((keyword("*") | keyword("/")) ~ unary).* >> {
    case (h, t) => t.foldLeft(h) {
  ...
  def keyword(str: String): Rule[String] = s(str) ~ ws >> { case (s, _) => s }

  def ws: Rule[Unit] = s(" ").* >> (_ => ())
}
